...
if __name__ == "__main__":
    ...
    batched_file_path = f"{data_dir}/all_batched.json"
    output_file_path = f"{data_dir}/all_single_fetched.json"
    uuid_table_path = f"{data_dir}/uuids.json"

    last_modified = isoparse("2000-01-01T00:00:00+00:00")
    data = []
    try:
        with open(batched_file_path) as f:
    ...
    subprocess.run([
        "./docker-run.sh",
        "--flow-name", flow_name,
        "--range-end-exclusive", last_modified.isoformat(),
        server,
        token,
        user,
        "all",

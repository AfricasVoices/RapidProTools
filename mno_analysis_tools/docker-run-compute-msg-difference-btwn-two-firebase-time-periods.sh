...
if [[ "$PROFILE_MEMORY" = true ]]; then
    PROFILE_MEMORY_CMD="mprof run -o /system-metrics/compute_msg_difference_btwn_two_firebase_time_periods_memory.prof"
fi
...
if [[ "$PROFILE_MEMORY" = true ]]; then
    echo "Copying $container_short_id:/system-metrics/compute_msg_difference_btwn_two_firebase_time_periods_memory.prof -> $MEMORY_PROFILE_OUTPUT_PATH"
    mkdir -p "$(dirname "$MEMORY_PROFILE_OUTPUT_PATH")"
    docker cp "$container:/system-metrics/compute_msg_difference_btwn_two_firebase_time_periods_memory.prof" "$MEMORY_PROFILE_OUTPUT_PATH"
fi

...
if [[ "$PROFILE_MEMORY" = true ]]; then
    PROFILE_MEMORY_CMD="mprof run -o /data/compute_messages_per_period_memory.prof"
fi
...
if [[ "$PROFILE_MEMORY" = true ]]; then
    echo "Copying $container_short_id:/data/compute_messages_per_period_memory.prof -> $MEMORY_PROFILE_OUTPUT_PATH"
    mkdir -p "$(dirname "$MEMORY_PROFILE_OUTPUT_PATH")"
    docker cp "$container:/data/compute_messages_per_period_memory.prof" "$MEMORY_PROFILE_OUTPUT_PATH"
fi
